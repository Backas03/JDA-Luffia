...
```
public static final String TRANSLATOR_URL = "http://192.168.0.221:11434|5080|qwen3.8:27b,http://127.0.0.1:8765|5600g";
```
가사 하단에는 ``(model: 모델, compute: 표시이름, process: 진행%, N token/s)`` 형태로 현재 번역 상태가 표시됩니다. process 는 현재 곡과 미리 번역해 두는 대기열 곡들의 전체 진행률입니다. 미리 번역은 첫 주소(GPU)가 살아 있으면 대기열 앞 20곡, 폴백(CPU) 상태면 3곡까지 해 둡니다.
